...
C1_height = size(C1, 1);
C1_width = size(C1, 2);
C1_S_edges = zeros(C1_height, C1_width);
C1_R_edges = zeros(C1_height, C1_width);

C2_height = size(C2, 1);
C2_width = size(C2, 2);
C2_S_edges = zeros(C2_height, C2_width);
C2_R_edges = zeros(C2_height, C2_width);

C3_height = size(C3, 1);
C3_width = size(C3, 2);
C3_S_edges = zeros(C3_height, C3_width);
C3_R_edges = zeros(C3_height, C3_width);
...
for y = 1:C3_height
    for x = 1:C3_width
        C3_S_edges(y, x) = round(sqrt(C3_Sx(y,x)^2 + C3_Sy(y,x)^2));
        C3_R_edges(y, x) = round(sqrt(C3_Rx(y,x)^2 + C3_Ry(y,x)^2));
    end
end
